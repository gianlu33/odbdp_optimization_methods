...
public class GeneticAlgorithm {
	...
	public GeneticAlgorithm(DataStructure data, String outputfile, int time) {
		this.data = data;
	...
	public void start(int numPopulation, int nElite, int pressure) {
		...
		generatePopulation(numPopulation);
		...
		while(System.currentTimeMillis() - timeStart < timeMillis) {
			...
			//numPopulation = numPopulation <= 50 ? numPopulation : numPopulation-2;
			//TODO verifica pressure
			if(generation % 5 == 0)
				pressure = pressure >= numPopulation / 5 ? numPopulation / 5 : pressure+1;
	...
	private void generatePopulation(int numPopulation) {
		boolean[] tempIndexes;
	 ...
	 */
	private Solution tournamentSelection(int numPopulation, int pressure) {
		int best = -1, temp;
	...
	private void printSimilarityMatrix(int numPopulation) {
		BufferedWriter bw;
		String s;
	...
	private void mutateEquals() {
		...
		for(int i=1; i<size; i++) {
			...
			if(checkSimilarity(s, sOld) > 0.9) {
				//mutate s
				boolean[] indexes = sOld.getIndexes();
				mutation(indexes, 5);
				sOld.copy(Utils.generateSolutionFromIndexes(data, indexes));
				firstImprovement(sOld);
			}
	...
	private class GAThread extends Thread{	
		private int numPopulation, pressure, generation;
		
		private GAThread(int numPopulation, int pressure, int generation) {
		...
		public void run() {
			...
			Solution s1 = tournamentSelection(numPopulation, pressure);
			Solution s2 = tournamentSelection(numPopulation, pressure);
			...
			//int mutationRate = 103 - Math.round(similarity * 100); //TODO verifica una funzione di similarity
			//int mutationRate = (int) Math.round(100 * Math.exp(-3 * similarity));
			//System.out.println(similarity + " " + mutationRate);
			
			int mutationRate =  Math.round(100f - similarity * 98); //TODO verifica una funzione di similarity
